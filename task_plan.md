...
### Phase 13: 多省份支持 ✅ 完成
- **状态**: complete
- **目标**: 从浙江省扩展到全国主要省份，支持三种高考模式
- **背景**: 当前仅支持浙江（新高考3+3），用户需求覆盖更多省份
- **三种高考模式**:

| 模式 | 代表省份 | 选科规则 | 总分 |
|------|----------|----------|------|
| 新高考 3+3 | 浙江（7选3）、山东（6选3） | 语数英 + 选3 | 750 |
| 新高考 3+1+2 | 江苏 | 语数英 + 物理/历史二选一 + 化生政地四选二 | 750 |
| 老高考（文理分科） | 预留接口（数据待补充） | 语数英 + 文综/理综 | 750 |

- **技术架构**:
  - 数据层：每省一个 JSON 文件（`data/{province}-data.json`），统一结构
  - 配置层：新增 `js/province-config.js`，定义各省模式/科目/总分/选科组合数
  - 应用层：`app.js` 动态加载对应省份数据，动态生成选科选择器
  - UI 层：省份选择器启用，文案动态化（移除硬编码"浙江省"）

- **子任务拆分**:
  1. **13.1 架构改造** ✅：抽取省份配置（PROVINCE_CONFIG），数据加载从固定路径改为 `fetch(config.dataFile)`
  2. **13.2 选科动态化** ✅：`buildSubjectSelector()` 根据模式动态生成选科选项
     - free 模式（3+3）：所有科目并列 checkbox，限制 electiveCount 个
     - layered 模式（3+1+2）：首选区 radio（限1）+ 再选区 checkbox（限2）
  3. **13.3 文案动态化** ✅：6处硬编码"浙江省"文案改为 `updateProvinceUI()` 动态生成
     - data-source-eyebrow / provinceHint / printProvince / disclaimer / FAQ / footer
  4. **13.4 江苏省数据** ✅：53所高校 / 764专业 / 1137条记录
     - 主数据源：江苏省教育考试院 2023-2025 投档线 XLS（www.jseea.cn）
     - 关键挑战：江苏投档表只公布分数不公布位次，通过一分一段表反查 score→rank
     - 位次补全率：97.9%（1113/1137），24条未补全为顶尖高分段超出一分一段表公布范围
  5. **13.5 山东省数据** ✅：64所高校 / 2198专业 / 5507条记录
     - 主数据源 A：山东省教育招生考试院 2023-2025 投档情况表 XLS（www.sdzk.cn）
     - 主数据源 B：山东一分一段表 XLS（投档表只有位次无分数，需 rank→score 反查）
     - 选科规则：6选3（物化生史地政，不含"技术"）
  6. **13.6 老高考接口预留** ✅：PROVINCE_CONFIG 中预留 examMode="old" 配置位，待数据补充后启用

- **数据来源（100% 官方可追溯）**:
  - 浙江：浙江省教育考试院（www.zjzs.net）— 83所/2358专业/5927记录 ✅
  - 山东：山东省教育招生考试院（www.sdzk.cn）— 64所/2198专业/5507记录 ✅
  - 江苏：江苏省教育考试院（www.jseea.cn）— 53所/764专业/1137记录 ✅
  - 详见 `data/DATA_SOURCES.md` 完整记录每个 XLS 的发布机构、URL、下载时间

- **验收标准达成情况**:
  1. ✅ 省份选择器可用，切换省份后数据/选科/文案全部刷新（onProvinceChange 链式触发）
  2. ✅ 3+3 模式（浙江7选3/山东6选3）— free 模式动态生成
  3. ✅ 3+1+2 模式（江苏）— layered 模式分层生成：首选 radio + 再选 checkbox
  4. ✅ 推荐算法自适应（位次法/冲稳保/概率计算无需修改，配置驱动）
  5. ✅ 数据来源 100% 官方可追溯（每条记录可回溯到省考试院 XLS）

- **端到端测试结果** (`data/raw/test_multi_province.js`)：
...
- **核心文件变更**：
  - 新建 `js/province-config.js`：PROVINCE_CONFIG 集中配置 + 全局函数
  - 修改 `index.html`：省份选择器启用、选科容器动态化、6处文案加 id
  - 修改 `js/app.js`：loadData 动态化、buildSubjectSelector/getSelectedSubjects 双模式、onProvinceChange/updateProvinceUI
  - 修改 `css/style.css`：新增 .subject-group / .subject-group-label / .subject-selector-row 分层选科样式
  - 新建 `data/jiangsu-data.json` / `data/shandong-data.json`
  - 更新 `data/DATA_SOURCES.md`：追加江苏、山东数据来源说明
