...
### Phase 14: 全国31省扩展 ✅ 完成
- **状态**: complete
- **目标**: 从3省扩展到全部31个高考省份，覆盖所有高考模式
- **背景**: 用户要求"全部省份"，确认范围=31省、官方数据第一、老高考纳入
- **全国高考模式分类**（基于教育部2025年官方信息）：
  - **3+3 模式（6省）**：浙江✅、山东✅、上海✅、北京✅、天津✅、海南✅
  - **3+1+2 模式（23省）**：江苏✅ + 第三批8省✅(2021起) + 第四批7省✅(2024起) + 第五批8省✅(2025起)
  - **老高考文理分科（2省）**：新疆✅、西藏✅
...
| 批次 | 省份 | 模式 | 数据年份 | 状态 |
|------|------|------|----------|------|
| 1 | 广东、河北、湖南、湖北、福建 | 3+1+2 | 2023-2025 | ✅ 完成 |
| 2 | 重庆、辽宁、安徽、江西、广西 | 3+1+2 | 2023-2025 | ✅ 完成 |
| 3 | 吉林、黑龙江、甘肃、贵州、上海 | 3+1+2/3+3 | 2024-2025/2023-2025 | ✅ 完成 |
| 4 | 河南、四川、山西、陕西、云南 | 3+1+2 | 2025(首届) | ✅ 完成 |
| 5 | 内蒙古、宁夏、青海、北京、天津 | 3+1+2/3+3 | 2025/2023-2025 | ✅ 完成 |
| 6 | 海南、新疆、西藏 | 3+3/老高考 | 2023-2025 | ✅ 完成 |

- **特殊处理已实现**：
  - 第五批8省（2025首届新高考）：只有2025年1年新高考数据
  - 上海总分660分（非750），选考科目满分70分（非100分），scoreLayout 配置
  - 海南总分900分（标准分转换），totalScore=900
  - 老高考省份（新疆/西藏）：electiveMode='old'，文/理科 radio 选择，5个核心函数添加 old 分支
- **关键 bug 修复**：
  - 湖南 2770 条记录 min_rank 全部为 null → 通过 hneeb.cn 一分一段表反查 → 100% 补全
  - CSS radio 按钮未隐藏（layered/old 模式 UI 破损）→ 新增 .subject-chip input[type="radio"] 选择器
  - index.html 3 个缺失 ID（printProvince/disclaimerText/footerText）→ 补全
- **验收标准达成**：
  1. ✅ 31省全部在省份选择器中可选（6个optgroup分组）
  2. ✅ 每省切换后数据/选科/文案全部刷新（updateProvinceUI 增强，动态调整 score input max）
  3. ✅ 数据来源100%官方可追溯（DATA_SOURCES.md 追加28省汇总表 + 已知限制）
  4. ✅ 端到端测试全部通过（四层验证：数据/配置/代码/HTML）
